...
if [[ -z "$MYSQL_ROOT_PASS" ]]; then
  dnf install -y openssl tar wget
...
sudo dnf install -y https://rpms.remirepo.net/enterprise/remi-release-9.rpm | tee -a "$LOG_FILE"
sudo dnf module reset php -y && sudo dnf module enable php:remi-8.2 -y | tee -a "$LOG_FILE"
sudo dnf install -y php php-{mbstring,mysqli,xml,cli,ldap,openssl,pecl-apcu,zip,curl,gd,intl,json,session,imap,zlib,redis} | tee -a "$LOG_FILE"
