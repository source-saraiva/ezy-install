...
if [ ! -d "$dotnetDir" ] && [ "$dotnetFound" = "yes" ]; then
  ...
  curl -sSL "$dotnetUrl" | sudo bash /dev/stdin -c "$dotnetVersion" --runtime aspnetcore --no-path --install-dir "$dotnetDir" --verbose >> "$installLog" 2>&1
...
if ! curl -o "/tmp/DnsServerPortable.tar.gz" --fail "$dnsUrl" >> "$installLog" 2>&1; then
  echo "Failed to download: $dnsUrl"
  exit 1
fi

echo "Extracting Technitium DNS Server..."
sudo cp "/tmp/DnsServerPortable.tar.gz" "$dnsTar"
